"""
Taken mostly from super-gradients
"""
import torch
...
class SegmentationHead(nn.Module):
    def __init__(self, in_channels: int, mid_channels: int, num_classes: int, dropout: float, bayes: bool):
        ...
        if bayes:
            self.conv_var = nn.Conv2d(mid_channels, num_classes, kernel_size=1, bias=False)
        else:
            self.conv_var = nn.Identity()

    def forward(self, x):
        feat =  self.seg_head(x)
...
class PPLiteSegBase(SegmentationModule):
    ...
    def __init__(
        ...
        self.seg_head = SegmentationHead(in_channels=decoder_channels[-1], mid_channels=head_mid_channels, num_classes=num_classes, dropout=dropout, bayes=self.bayes)
        self.out_upsample = make_upsample_module(scale_factor=head_scale_factor, upsample_mode=head_upsample_mode, align_corners=align_corners)
        self.adf_softmax = ADFSoftmax() if self.apply_adf_softmax else nn.Identity()
        # Auxiliary heads
    ...
    def forward(self, x):
        ...
        out, var = self.seg_head(x)
        if self.bayes:
            if self.apply_adf_softmax:
                out, var = self.adf_softmax(out, var)
            out = self.out_upsample(out)
            var = self.out_upsample(var)
            return out, var
        # otherwise just upsample the normal output
        out = self.out_upsample(out)
        if not self.use_aux_heads:
            return out
        aux_feats = [aux_head(feat) for feat, aux_head in zip(enc_feats, self.aux_heads)]
        return tuple([out] + aux_feats)
