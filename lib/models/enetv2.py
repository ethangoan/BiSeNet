...
# Using custom maxunpool due to compatability with onnx
# https://github.com/pytorch/pytorch/issues/25088#issuecomment-1090956803
...
class ENet(nn.Module):
    ...
    def __init__(self, num_classes, encoder_relu=False, decoder_relu=True, aux_mode='train'):
        super().__init__()
    ...
    def forward(self, x):
        ...
        if self.bayes:
            var = self.out_var(x ** 2.0)
            if self.apply_adf_softmax:
                out, var = self.adf(out, var)
            return out, var
        else:
            # otherwise just return normal output
            return out
